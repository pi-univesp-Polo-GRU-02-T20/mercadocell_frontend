...
function categoriaPDF(vetor){
    ...
    const dados = vetor.map((converter) => {
        return [
            {text: converter.codCategoria, fontSize: 9, margin: [0, 2, 0, 2], border: [false, true, false, true]},     //Colocar nome utilizado no banco de dados
    ...
    const details = [
        ...
        {text: 'LISTA DE CATEGORIAS', style: 'subheader', margin: [0,30,10,20], bold: true, border: [false, true, false, true]},
        {
            table:{
                headerRows: 1,
   ...
   //       layout: 'lightHorizontalLines',
    ...
    // consulta-produto_20220605_01_18.pdf
    pdfMake.createPdf(docDefinitios).download('consulta-categoria_' + d + '_' + h + m + '.pdf');
}
