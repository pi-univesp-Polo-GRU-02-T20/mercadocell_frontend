...
function produtoPDF(vetor){
    ...
    const reportTitle = [
        {
            text: 'Produtos',                                                    //Colocar título correspondente ao documento
            fontSize: 15,
            bold: true,
            margin: [15, 20, 0, 45] 
        }
    ...
    const dados = vetor.map((converter) => {
        return [
            {text: converter.codProduto, fontSize: 9, margin: [0, 2, 0, 2]},     //Colocar nome utilizado no banco de dados
            {text: converter.nomeProduto, fontSize: 9, margin: [0, 2, 0, 2]},
            {text: converter.descricaoProduto, fontSize: 9, margin: [0, 2, 0, 2]},
        ] 
    });
